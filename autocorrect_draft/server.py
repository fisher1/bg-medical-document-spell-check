...
for _, unigram in unigrams.iterrows():
    ...
    for i in range(0, len(word)):
        ...
        if len(unigram_buckets[word_cut]) < 10:
            unigram_buckets[word_cut].append([-len(word), count, word])
...
def get_dash_word_suggestions(curr_word, curr_suggestions):
    ...
    for word in words:
        exists_in_dictionary = (unigrams['Word'] == word).any()
    ...
    suggestion_combinations.remove(curr_word)
...
def give_suggestions():
    ...
    return get_suggestions(curr_word, preceeding_word, 8)
...
def correct_text():
    ...
    for i in range(0, len(tokens)):
        ...
        exists_in_dictionary = (unigrams['Word'] == token).any() 
